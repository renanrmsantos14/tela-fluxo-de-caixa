...
function validationBody(entry: CashflowEntry, validatedAt: string, ruleReference?: string): Record<string, unknown> {
  ...
  body['cr40f_ContraparteRef@odata.bind'] = entry.counterpartyId
    ? `/cr40f_fluxocaixacontrapartes(${entry.counterpartyId})`
    : null;
...
function ruleBody(rule: ClassificationRule): Record<string, unknown> {
  const body: Record<string, unknown> = {
  ...
  if (rule.counterpartyId) body['cr40f_ContraparteRef@odata.bind'] = `/cr40f_fluxocaixacontrapartes(${rule.counterpartyId})`;
  return body;
}
