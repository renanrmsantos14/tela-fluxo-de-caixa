...
import { ArrowDownRight, ArrowUpRight, Banknote, CalendarDays, CheckCircle2, ChevronRight, CircleAlert, Clock3, FileUp, Landmark, Menu, Plus, RefreshCw, Settings2, Sparkles, Upload, WalletCards, X, Tags, Repeat2, Building2, Undo2, Save, SlidersHorizontal, ScrollText } from 'lucide-react';
import './styles.css';
...
type View = 'flow' | 'imports' | 'accounts' | 'categories' | 'recurrences' | 'audit' | 'settings';
...
function App() {
  ...
  useEffect(() => {
    async function hydrate() {
      ...
      try {
        const [, mapping, recurring, loadedAccounts, loadedCategories, loadedRecurrences, loadedHolidays, loadedSettings, loadedRules, loadedAuditEvents] = await Promise.all([loadEntries(resolved), loadOrderMapping(resolved), loadRecurringForecasts(resolved, buildWeeks(new Date(), 26).at(-1) ?? new Date()), listReferences(resolved, financeSets.accounts, 'cr40f_fluxocaixacontaid,cr40f_name,cr40f_banco,cr40f_identificador'), listReferences(resolved, financeSets.categories, 'cr40f_fluxocaixacategoriaid,cr40f_name,cr40f_grupo,cr40f_natureza'), listReferences(resolved, financeSets.recurrences, 'cr40f_fluxocaixarecorrenciaid,cr40f_name,cr40f_valor,cr40f_categoria,cr40f_natureza,cr40f_frequencia,cr40f_intervalodias,cr40f_inicio,cr40f_fim'), listReferences(resolved, financeSets.holidays, 'cr40f_fluxocaixaferiadoid,cr40f_name,cr40f_data'), listReferences(resolved, financeSets.settings, 'cr40f_fluxocaixaconfiguracaoid,cr40f_name,cr40f_entidadeop,cr40f_entitysetop,cr40f_campoidop,cr40f_campovalorop,cr40f_campodataop,cr40f_destinatariosalerta'), listReferences(resolved, financeSets.rules, 'cr40f_fluxocaixaregraid,cr40f_name,cr40f_expressao,cr40f_categoria'), listReferences(resolved, financeSets.events, 'cr40f_fluxocaixaeventoid,cr40f_name,cr40f_acao,cr40f_detalhe,cr40f_data')]);
        setAccounts(loadedAccounts); setCategoryCatalog(loadedCategories); setRecurrences(loadedRecurrences); setHolidays(loadedHolidays); setSettings(loadedSettings[0] ?? null); setRules(loadedRules); setAuditEvents(loadedAuditEvents);
  ...
  function importFile(event: ChangeEvent<HTMLInputElement>) {
    ...
    file.text().then(parseOfx).then((result) => {
      ...
      setAccount(accounts.find((item) => item.identifier === result.accountId)?.name ?? '');
  ...
  async function confirmOfx() {
    ...
    if (!account) return notify('Selecione uma conta bancária cadastrada antes de importar.');
    ...
    await audit(context, 'Importação OFX', `${imported.length} transações importadas da conta ${account}.`).catch(() => undefined);
  ...
  async function createManual(event: FormEvent<HTMLFormElement>) {
    ...
    const entry: CashflowEntry = {
      ...
      nature: String(form.get('nature')) as CashflowEntry['nature'], status: 'open', source: 'manual', account: String(form.get('account'))
    };
    try { await saveEntry(context, entry); await audit(context, 'Previsão manual', `${entry.description} criada para ${entry.date}.`); } catch { notify('Previsão salva localmente; persistência Dataverse indisponível neste contexto.'); }
  ...
  return <div className="app-shell">
    <aside className="sidebar" aria-label="Navegação principal">
      ...
      <nav>{([
        ['flow', Landmark, 'Fluxo de caixa'], ['imports', FileUp, 'Importações OFX'], ['accounts', Building2, 'Contas'], ['categories', Tags, 'Categorias'], ['recurrences', Repeat2, 'Recorrências'], ['audit', ScrollText, 'Auditoria'], ['settings', Settings2, 'Configurações']
    ...
    {modal === 'manual' && <ManualModal accounts={accounts} categories={categoryCatalog} onClose={() => setModal(null)} onSubmit={createManual} />}
    {modal === 'ofx' && <OfxModal accounts={accounts} ofx={ofx} account={account} onAccount={setAccount} onClose={() => { setModal(null); setOfx(null); setOfxFile(null); }} onSelect={importFile} onConfirm={confirmOfx} />}
